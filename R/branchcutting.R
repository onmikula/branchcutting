...
#'   a list with components `support` (support for various delimited species), `classification`
#'   (definition of the species) and `consensus` (data frame with the consensus species delimitation).	
#' @examples
...
branchcutting <- function(phy, psample, heterosp=NULL, consp=NULL, rescale=TRUE, bw="mdif", kernel="epanechnikov", outgroup=NULL) {
	...
	if (!is.null(psample)) {
		...
		map <- matrix(0, length(species), length(tips), dimnames=list(names(species), tips))
		for (i in seq_along(species)) {
			map[i,species[[i]]] <- 1
		}
		psample <- list(support=spdf, classification=map)
		psample$consensus <- data.frame(ID=unlist(species[spdf$Consensus]), Species=rep(names(species[spdf$Consensus]), sapply(species[spdf$Consensus], length)), stringsAsFactors=FALSE, row.names=NULL)
		if (!is.null(phy)) {
